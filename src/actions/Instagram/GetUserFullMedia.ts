...
async function GetUserMediaChildrenObjects(media_id: string, token: string): Promise<any> {
  try {
...
async function GetUserCarouselMediaData(media_id: string, token: string): Promise<any> {
  try {
...
export default async function GetUserFullMedia(type: string, media_id: string, token: string): Promise<any[]> {
  try {
    if (type === "CAROUSEL_ALBUM") {
      ...
      for (const item of mediaChildren.data) {
        const carouselMediaData = await GetUserCarouselMediaData(item.id, token);
